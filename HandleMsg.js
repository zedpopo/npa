...
module.exports = HandleMsg = async (aruga, message) => {
            ...
            if (isOwnerBot) {
                await aruga.joinGroupViaLink(linkgrup)
                      .then(async () => {
                          ...
                          await aruga.sendText(chekgrup.id, `Hai minna~, Im Popox Bot. To find out the commands on this Bot type ${prefix}menu`)
                      })
            } else {
	...
	case 'cooltext':
        ...
        case 'stiker':
            if ((isMedia || isQuotedImage) && args.length === 0) {
                ...
                .then(() => {
                    aruga.reply(from, 'Here\'s Sticker lo asu!')
	...
	case 'brainly':
            ...
            if (args.length >= 2){
			    ...
			    aruga.sendText(from, 'Selesai ✅, donasi kesini ya Dana : 081278244595 | Pulsa : 081278244595')
                        } else {
        ...
        case 'stikergif':
            if (isMedia || isQuotedVideo) {
                if (mimetype === 'video/mp4' && message.duration < 10 || mimetype === 'image/gif' && message.duration < 10) {
                    var mediaData = await decryptMedia(message, uaOverride)
                    aruga.reply(from, '[WAIT] Sedang di proses⏳ silahkan tunggu nyet ± 1 min!', id)
	...
	if (!isBotGroupAdmins) return aruga.reply(from, 'Perintah ini hanya bisa di gunakan ketika bot menjadi admin', id)
		...
		case 'facebook':
			...
			.then(async (res) => {
				...
				if (res.status == 'error') return aruga.sendFileFromUrl(from, link, '', 'Mff iyh url anda tidak dapat ditemukan', id)
				await aruga.sendFileFromUrl(from, linkhd, '', 'Nih ngab videonya', id)
				.catch(async () => {
					await aruga.sendFileFromUrl(from, linksd, '', 'Nih ngab videonya', id)
					.catch(() => {
						aruga.reply(from, 'Mff iyh url anda tidak dapat ditemukan', id)
					})
				})
			})
			break
	...
	case 'dewabatch':
        ...
        case 'ss': //jika error silahkan buka file di folder settings/api.json dan ubah apiSS 'API-KEY' yang kalian dapat dari website https://apiflash.com/
            ...
            await aruga.sendFile(from, scrinshit, 'ss.jpg', 'cekrek cekrek duar mati lampu', id)
            .catch(() => {
                aruga.reply(from, 'Ada yang Error!', id)
            })
            break
            ...
            if (isMedia && type === 'image' || quotedMsg && quotedMsg.type === 'image') {
                ...
                .then(resolt => {
                	if (resolt.docs && resolt.docs.length <= 0) {
                		aruga.reply(from, 'Maaf, saya tidak tau ini anime apa, pastikan gambar yang akan di Search tidak seperti muka anda/Kepotong', id)
                	}
        ...
        case 'tts':
            ...
            const dataText = body.slice(8)
                if (dataText === '') return aruga.reply(from, 'apa teksnya anjg?..', id)
                try {
    ...
    if (!isGroupMsg) return aruga.reply(from, menuId.textPrem())
            ...
            .catch(() => {
                aruga.reply(from, 'Hayolohhh, ada yang error tolol!!', id)
            })
            break
    case 'dare':
    if (!isGroupMsg) return aruga.reply(from, menuId.textPrem())
		...
		case 'klasmen':
            ...
            let textKlas = "*BAYAR UTANG LO KNTL*\n"
            let i = 1;
            urut.forEach((klsmn) => {
		...
		case 'simisimi':
			if (!isGroupMsg) return aruga.reply(from, 'Maaf, perintah ini hanya dapat dipakai didalam grup!', id)
			aruga.reply(from, 'jANGAN DI NYALAIN SIMI SIMINYA ASU !!! ', id)
			break
		case 'simi':
			if (!isGroupMsg) return aruga.reply(from, 'Maaf, perintah ini hanya dapat dipakai didalam grup!', id)
            if (!isGroupAdmins) return aruga.reply(from, 'Gagal, perintah ini hanya dapat digunakan oleh admin grup!', id)
			...
			if (args[0] == 'on') {
                ...
                aruga.reply(from, 'SAATNY KELUAR DARI GRUOP!!! MATITTIN ASU!', id)
			} else if (args[0] == 'off') {
		...
		case 'kasar':
			...
			if (args[0] == 'on') {
				...
				aruga.reply(from, 'Fitur Anti Kasar sudah di Aktifkan, jANGAN LUPA BAYAR HUTANG!', id)
			} else if (args[0] == 'off') {
        ...
        case 'bc': //untuk broadcast atau promosi
            ...
            for (let idk of chatz) {
                ...
                if (!cvk.isReadOnly) aruga.sendText(idk, `〘 *P O P O X  B C* 〙\n\n${msg}`)
                if (cvk.isReadOnly) aruga.sendText(idk, `〘 *P O P O X  B C* 〙\n\n${msg}`)
            }
		...
		if(!isCmd && isGroupMsg && isNgegas) {
            ...
            if(find && find.id === groupId){
                ...
                if(cekuser && isIn !== false){
                    if(isKasar){
                        const denda = db.get('group').filter({id: groupId}).map('members['+isIn+']').find({ id: pengirim }).update('denda', n => n + 10000).write()
                        if(denda){
                            await aruga.reply(from, "Jangan badword monyet! \nDenda +10.000\nTotal : Rp"+formatin(denda.denda), id)
                        }
                    }
                } else {
                    ...
                    if(cekMember.length === 0){
                        if(isKasar){
                            db.get('group').find({ id: groupId }).set('members', [{id: pengirim, denda: 10000}]).write()
                        } else {
                        ...
                        const cekuser = db.get('group').filter({id: groupId}).map('members').value()[0]
                        if(isKasar){
                            cekuser.push({id: pengirim, denda: 10000})
                ...
                if(isKasar){
                    db.get('group').push({ id: groupId, members: [{id: pengirim, denda: 10000}] }).write()
                    await aruga.reply(from, "Jangan badword monyet! \nDenda +10.000\nTotal : Rp10.000", id)
                } else {
